...
//    comm_nr[dir] = op->c.num_boundary_sites[2*dir +1]; //This is not correct!
  ...
  int *boundary_table;// = op->c.boundary_table[0];
  int bt_index; 
  for (dir = T; dir <= X; dir++){
    ...
    bt_index = 0;
    neighbors_j_start = l->neighbor_rank[2*dir] * l->num_inner_lattice_sites * num_site_var;
    for (node = core_start; node < core_end; node ++){
      index = 5 * node;
...
*/
    ...
    if (comm_nr[dir] > 0){
      ...
      if ( g.my_rank == 0){							// tag
	for (k = 0; k < comm_nr[dir]; k ++){
/*		for (j = 0; j < num_link_var; j++){
//			printf("%1.3f, %1.3f", creal(*(buff_d_send[dir] + num_link_var * k +j)), cimag(*(buff_d_send[dir] + num_link_var * k +j)));//(k+1)-1
			printf("%1.2f, ", creal(*(buff_d_send[dir] + num_link_var * k +j)));
			if ((j+1) % num_site_var == 0) printf("\n");
		}
		printf("\n\n");			*/
//			buff_d works

//		printf("Buff_i[%d]: %d, %d\n", k, buff_i_send[dir][2*k], buff_i_send[dir][2*k + 1]);
//			buff_i works
	}
      }
...
//			len					[T+, T-, Z+, Z-...]
	...
	for (k = 0; k < comm_nr[dir]; k++){
/*
		for (j = 0; j < num_link_var; j+= 12){
//			printf("%1.3f, %1.3f", creal(*(buff_d_send[dir] + num_link_var * k +j)), cimag(*(buff_d_send[dir] + num_link_var * k +j)));//(k+1)-1
			printf("%1.2f, ", creal(*(buff_d_recv[dir] + num_link_var * k +j)));
			if ((j + 12) % (num_site_var) == 0) printf("\n");
		}
		printf("\n\n");
*/
/*
		printf("post recv.. at %d, \tfrom ", *(buff_i_recv[dir]+ 2* k));
		printf("%d, \t", *(buff_i_recv[dir] + 2* k +1));
		printf("%f", creal(*(buff_d_recv[dir] + num_link_var * (k+1) -1)));
		printf("\n");
*/
	}
      }
...
//FIXME start locked master
// make it work
/*
  for (dir = X; dir >= T; dir --){
	FREE(buff_i_send[dir], int, 2 * comm_nr[dir]);
	FREE(buff_i_recv[dir], int, 2 * comm_nr[dir]);
	FREE(buff_d_send[dir], complex_PRECISION, num_link_var * comm_nr[dir]);
	FREE(buff_d_recv[dir], complex_PRECISION, num_link_var * comm_nr[dir]);
  }
  exit(0);
*/
